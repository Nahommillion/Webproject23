...
function confetti(){const target=(!$('fullscreenGame').classList.contains('hidden')?$('fullscreenGame'):document.body);for(let i=0;i<80;i++){const s=document.createElement('i');s.className='confetti';s.style.left=Math.random()*100+'vw';s.style.setProperty('--h',Math.floor(Math.random()*360));s.style.animationDelay=Math.random()*.6+'s';target.appendChild(s);setTimeout(()=>s.remove(),2000)}}
function fsNewGame(){
  if(spinning)return;
