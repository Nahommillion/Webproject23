...
function renderFullscreenEntries(){const box=$('fsEntries');if(!box)return;const count=$('fsEntryCount');if(count)count.textContent=`(${entries.length})`;box.innerHTML=entries.map((x,i)=>`<div class="fsEntry ${i===selected?'active':''}"><span class="num">${i+1}</span><span class="entryName">${escapeHtml(x)}</span></div>`).join('')}
